...
def plot_plasma(filepath = './',fig = None,ax = None):
    ...
    sep_plt_check = plt.plot(edge_X,edge_Z,'-.',lw = 3)
...
def oneoff_settings(case,fig,ax,pts):
    if case == 'standard_diverted':
        ax.annotate('($R_{min},Z_{R_{min}}$)',(pts[0][0]+.01,pts[0][1]-.015),textcoords = 'data',size  = 25,fontweight = 'bold')
        plt.yticks([])

    if case  == 'standard_circular':
        fig.subplots_adjust(left = .05,right = .99,top = .975,bottom = .025)
        ax.annotate('($R_{min},Z_{R_{min}}$)',(pts[0][0]+.01,pts[0][1]-.005),textcoords = 'data',size  = 25,fontweight = 'bold')
        plt.yticks(fontsize = 15)
        plt.ylabel('Z (m)',fontsize = 20)

    ax.annotate('($R_{max},Z_{R_{max}}$)',(pts[1][0]-.08,pts[1][1]-.005),textcoords = 'data',size  = 25,fontweight = 'bold')        
    ax.annotate('($R_{Z_{min}},Z_{min}$)',(pts[2][0]-.03,pts[2][1]+.025),textcoords = 'data',size  = 25,fontweight = 'bold')        
    ax.annotate('($R_{Z_{max}},Z_{max}$)',(pts[3][0]-.03,pts[3][1]-.025),textcoords = 'data',size  = 25,fontweight = 'bold')

    plt.xticks(fontsize = 15)
    plt.xlabel('R (m)',fontsize = 20)
    

fig = plt.figure(8,figsize = (25,11))

ax1 = fig.add_subplot(121,aspect = 'equal')
plot_plasma(filepath = './TokaMac/Circular_shot_70000/',fig = fig,ax = ax1)

ax2 = fig.add_subplot(122,aspect = 'equal')
plot_plasma(filepath = './TokaMac/Diverted_low_ip_May_2016',fig = fig,ax = ax2)

fig.subplots_adjust(left = .05,right = .99,top = .975,bottom = .1,wspace = .01)
